...
(function(expose) {
 ...
 /**
  ...
  var Yfhs = DialectHelpers.subclassDialect( Gruber ),
    extract_attr = MarkdownHelpers.extract_attr,
    forEach = MarkdownHelpers.forEach;



  Yfhs.inline[ "@" ] = function mention(text){
    var m = text.match(/@(\w{1,40})/);
    if ( m ) {
      return [m[0].length, ['a', {"class": 'mention', 'href': 'users/' + m[ 1 ]}, m[ 0 ]]];
    } else {
      return [1, "@"];
    }
  };
  ...
  Markdown.dialects.Yfhs = Yfhs;
  Markdown.dialects.Yfhs.inline.__escape__ = /^\\[\\`\*_{}\[\]()#\+.!\-|:]/;
  Markdown.buildBlockOrder ( Markdown.dialects.Yfhs.block );
  Markdown.buildInlinePatterns( Markdown.dialects.Yfhs.inline );
